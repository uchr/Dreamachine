...
std::optional<std::filesystem::path> exportAlphaTexture(const std::filesystem::path& path) {
    auto isPathContainSubstring = [&](const std::vector<std::string>& substrings) {
        auto substringIt = std::find_if(substrings.begin(), substrings.end(), [&](const std::string& substring) {
            return path.filename().string().find(substring) != std::string::npos;
        });
...
struct NMLImage {
    ...
    void set(int x, int y, uint32_t value) { data[x + y * sizeX] = value; }
...
struct Image16 {
    ...
    int numberOfBytes() const { return sizeX * sizeY * 2; }

    float msb(int x, int y) { return (float)(data[x + y * sizeX] / 0x100) / 255.0f; }

    float lsb(int x, int y) { return (float)(data[x + y * sizeX] & 0xFF) / 255.0f; }

    float med(int x, int y) { return (msb(x, y) + lsb(x, y)) * 0.5f; }
...
bool loadNML(const std::filesystem::path& path, const std::filesystem::path& exportPath) {
    ...
    for (int i = 0; i < mipLevels; i++) {
        ...
        int result = stbi_write_png(exportPath.string().c_str(),
                                    nmlImage.sizeX,
                                    nmlImage.sizeY,
                                    4,
                                    reinterpret_cast<const char*>(nmlImage.data.data()),
                                    0);
        if (result > 0)
            return true;
    }
...
void parseTextures(MeshPart& meshPart, const std::vector<std::filesystem::path>& texturesPath, const std::filesystem::path& exportFolder) {
    ...
    for (int i = 0; i < texturesPath.size(); ++i) {
        ...
        if (!isLoaded) {
            ...
            if (!SUCCEEDED(hr))
                hr = DirectX::LoadFromDDSFile(widen(texturesPath[i].string()).c_str(),
            ...
            if (SUCCEEDED(hr)) {
                isLoaded = true;
                hr = DirectX::SaveToWICFile(*imageData.GetImage(0, 0, 0),
                                            DirectX::WIC_FLAGS_NONE,
                                            DirectX::GetWICCodec(DirectX::WIC_CODEC_PNG),
                                            widen(exportPath.string()).c_str());
            }
